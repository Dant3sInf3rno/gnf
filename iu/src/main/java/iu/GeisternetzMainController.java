...
@Named
@RequestScoped
public class GeisternetzMainController {
    ...
    public String save() {
        ...
        return "netzebergenmain.xhtml?faces-redirect=true";  // Weiterleitung nach dem Speichern
    }
    ...
    public String deleteGeisternetz(int id, String status) {
        if (geisternetzDAO.deleteGeisternetzByIdAndStatus(id, status)) {
            return "netzebergenmain.xhtml?faces-redirect=true";  // Seite neu laden nach erfolgreichem Löschen
        } else {
    ...
    public String updateStatus() {
        ...
        if (selectedGeisternetzId != null && newStatus != null && !newStatus.isEmpty()) {
            if (geisternetzDAO.updateGeisternetzStatus(selectedGeisternetzId, newStatus)) {
                return "netzebergenmain.xhtml?faces-redirect=true";  // Seite neu laden nach Statusänderung
            }
        }
